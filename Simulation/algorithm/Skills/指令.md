...
```
请在 G:\Cursor\kato-modbus\Simulation\algorithm\simulation\06_1DCNN\
目录下创建 train_f411_1dcnn.py，内容如下：
```
...
def main():
    ...
    te_preds = te_probs.round().astype(np.int64)
    ...
    plot_model_comparison(
        ...
        flash_pc=100866*4/1024, flash_kb=flash_kb
    )
...
跑完会输出参数量、Flash估算、测试准确率，还会自动导出`f411_model.onnx`，这个文件就是后续导入X-CUBE-AI的输入。把结果贴给我。